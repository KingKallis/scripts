...
//  TF1 *all = new TF1("all","t0+t1+t2+t3+t4+t5+t6+t7+t8+t9+t10+t11+t12+t13+t14+t15+t16+t17+t18+bkgY",low,hi);
  TF1 *all = new TF1("all","t0+t1+t2+t3+t4+t5+t6+t7+t8+t9+t10+t11",low,hi);
// TF1 *all = new TF1("all","t12+t13+t14+t15+t16+t17+t18+t19+t20+t21+t22",low,hi);
  for(Int_t i=firstpeak;i<nrofpeaks;i++){
 
     sprintf(name,"all->SetParLimits(0+3*%i,0,10000);",i);
...
//
  ...
  for(Int_t i=firstpeak;i<nrofpeaks;i++){
     ...
     cout << endl << "peak #"<< i+1 << ": position = " << par[3*i+1] <<" MeV     FWHM = " << 1000*2.35*par[3*i+2] << " kev   peak counts ="<< peakcounts[i]  << "   (NNDC: "<< centroid[i] <<" MeV ;  "<< 1000*2.35*sigma[i] <<" keV ) "
<< endl;
     cout << " --------------> NNDC-peak centroid = "<< (par[3*i+1]-centroid[i])*1000 << " keV " << endl;
