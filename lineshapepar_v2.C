...
//=============================================================
 ...
 //ToF vs X1 histogram
  TH2F *htofvsX1_Y1new = new TH2F("htofvsX1posO","tof vs X1_Y1new",700,100,800,100,1910,2010);
 
  if(Mg24_NoCol)
    {
      DATA->Draw("tof:X1posO>>htofvsX1posO","X1flag==0 && U1flag==0 && Alphas_24Mg_NoCol && Cut_pad1X1_24Mg_NoCol","col");
    }
  else if(Mg24_Col)
    {
      DATA->Draw("tof:X1posO>>htofvsX1posO","X1flag==0 && U1flag==0 && Alphas_24Mg_Col && Cut_pad1X1_24Mg_Col","col");
    }

   htofvsX1posO->GetXaxis()->SetTitle("X1posO position (mm)");
   htofvsX1posO->GetYaxis()->SetTitle("ToF");
   c5 -> Update();
   c5 -> Modified();


  TH2F *hPeakofInterest = new TH2F("hPeakofInterest","tof vs X1posO",40,-20,20,100,-50,50);
  ...
  if(Mg24_NoCol)
    {
      DATA->SetAlias("X1posOat0",Form("X1posO-%g",X1mean)); //define the X1posO of the peak of interest to 0
      DATA->SetAlias("tofat0",Form("tof-%g",tofmean));  //define the tof of the peak of interest to 0
      DATA->Draw("tofat0:X1posOat0>> hPeakofInterest","X1flag==0 && U1flag==0 && Alphas_24Mg_NoCol && Cut_pad1X1_24Mg_NoCol","col"); 
    }
  else if(Mg24_Col)
    {
      DATA->SetAlias("X1posOat0",Form("X1posO-%g",X1mean)); //define the X1posO of the peak of interest to 0
      DATA->SetAlias("tofat0",Form("tof-%g",tofmean));  //define the tof of the peak of interest to 0
      DATA->Draw("tofat0:X1posOat0>> hPeakofInterest","X1flag==0 && U1flag==0 && Alphas_24Mg_Col && Cut_pad1X1_24Mg_Col","col"); 
    }  

  hPeakofInterest->GetXaxis()->SetTitle("X1posO-X1mean (mm)");
...
//=============================================================================================
 ...
 //implement lineshaping correction
  ...
  // DATA->SetAlias("X1new",Form("X1posO-(%g+(%g*(tof-%g)+(%g*pow(tof-%g,2))))",b0,b1,tofmean,b2,tofmean));
   DATA->SetAlias("X1new",Form("X1posO-((%g*(tof-%g)+(%g*pow(tof-%g,2)))*(X1posO-%g)/(%g))",b1,tofmean,b2,tofmean,X1ref,X1mean-X1ref));
...
//=============================================================
 ...
 //ToF vs X1 histogram
  TH2F *hY1vsX1posO = new TH2F("hY1vsX1new","Y1 vs X1new",700,100,800,480,-60.,60.);
 
  if(Mg24_NoCol)
    {
      DATA->Draw("Y1:X1new>> hY1vsX1new","X1flag==0 && U1flag==0 && Alphas_24Mg_NoCol && Cut_pad1X1_24Mg_NoCol","col");
    }
  else if(Mg24_Col)
    {
      DATA->Draw("Y1:X1new>> hY1vsX1new","X1flag==0 && U1flag==0 && Alphas_24Mg_Col && Cut_pad1X1_24Mg_Col","col");
    }

   hY1vsX1posO->GetXaxis()->SetTitle("X1new position (mm)");
   hY1vsX1posO->GetYaxis()->SetTitle("Y1 position");
   c1 -> Update();
   c1 -> Modified();


  TH2F *hPeakofInterest_Y1X1 = new TH2F("hPeakofInterest_Y1X1","Y1 vs X1new",40,-20,20,480,-60.,60.);
  ...
  if(Mg24_NoCol)
    {
      DATA->SetAlias("X1newat0",Form("X1new-%g",X1mean)); //define the X1posO of the peak of interest to 0
      DATA->Draw("Y1:X1newat0>> hPeakofInterest_Y1X1","X1flag==0 && U1flag==0 && Alphas_24Mg_NoCol && Cut_pad1X1_24Mg_NoCol","col"); 
    }
  else if(Mg24_Col)
    {
      DATA->SetAlias("X1newat0",Form("X1new-%g",X1mean));  //define the X1posO of the peak of interest to 0
      DATA->Draw("Y1:X1newat0>> hPeakofInterest_Y1X1","X1flag==0 && U1flag==0 && Alphas_24Mg_Col && Cut_pad1X1_24Mg_Col","col"); 
    }  

  hPeakofInterest_Y1X1->GetXaxis()->SetTitle("X1new-X1mean (mm)");
...
//=============================================================================================
 ...
 //implement lineshaping correction
   ...
   DATA->SetAlias("X1newcorr",Form("X1new-(%g+(%g*(Y1))+(%g*pow(Y1,2)))",a0,a1,a2));

   TH2F *hY1vsX1newcorr = new TH2F("hY1vsX1newcorr","Y1 vs X1newcorr",700,100,800,480,-60,60);

   TCanvas *c4 = new TCanvas("c4","Y1 vs X1 corrected matrix",10,10,900,600);

  if(Mg24_NoCol)
    {
      DATA->Draw("Y1:X1newcorr>> hY1vsX1newcorr","X1flag==0 && U1flag==0 && Alphas_24Mg_NoCol && Cut_pad1X1_24Mg_NoCol","col"); 
    }
  else if(Mg24_Col)
    {
      DATA->Draw("Y1:X1newcorr>> hY1vsX1newcorr","X1flag==0 && U1flag==0 && Alphas_24Mg_Col && Cut_pad1X1_24Mg_Col","col"); 
    }

   hY1vsX1newcorr->GetXaxis()->SetTitle("Corrected X1 position (mm)");
   hY1vsX1newcorr->GetYaxis()->SetTitle("Y1 position");
   c4 -> Update();
   c4 -> Modified();
